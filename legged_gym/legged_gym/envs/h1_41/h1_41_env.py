...
##### reward designs #####
    ...
    #     """
    #     开始训练时,关节移动较小,sup_loss较小,不必调大sup_coefficient
    #     """
    #     _reward =  torch.sum(torch.square(self.dof_pos[:,self.finger_indices] - self.default_dof_pos[:,self.finger_indices]), dim=1)
    #
    #     # pos_err = torch.norm(self.dof_pos[:,self.finger_indices] - self.default_dof_pos[:,self.finger_indices], p=1, dim=1)
    #     # # # print(pos_err[:10])
    #     # _reward = torch.exp(- pos_err)
    #     return _reward